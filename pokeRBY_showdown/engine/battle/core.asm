...
.next3
	...
	jr z, .next4
	ld a, [wLinkState]
	and a                       ; any non-zero link state -> skip switch prompt
	jr nz, .next4
	ld a, [wOptions]
...
; either the bag (normal battle) or bait (safari battle) was selected
	ld a, [wLinkState]
	and a                       ; any non-zero link state -> no items
	jr z, .notLinkBattle
...
LinkBattleExchangeData:
	ld a, [wShowdownConnected]
	and a
	jr z, .originalExchange
	callfar ShowdownExchangeData
	ret
.originalExchange
	ld a, $ff
	ld [wSerialExchangeNybbleReceiveData], a
...
.notDone
	...
	ld [wPlayerNumHits], a
.executeOtherEffects
	; In Showdown mode, only execute side effects if the server says they should happen.
	; wSD_Flags bit4 = M1 side effect, bit5 = M2 side effect
	ld a, [wShowdownConnected]
	and a
	jr z, .playerNormalSideEffects
	; Player is executing: if hWhoseTurn == wSD_WhoFirst, this is M1; else M2
	ld a, [hWhoseTurn]
	ld b, a
	ld a, [wSD_WhoFirst]
	cp b
	ld a, [wSD_Flags]
	jr nz, .playerCheckM2SE
	bit 4, a ; M1 side effect flag
	jp z, ExecutePlayerMoveDone
	jr .playerNormalSideEffects
.playerCheckM2SE
	bit 5, a ; M2 side effect flag
	jp z, ExecutePlayerMoveDone
.playerNormalSideEffects
	ld a, [wPlayerMoveEffect]
	and a
	jp z, ExecutePlayerMoveDone
...
.IsConfused
	...
	ld a, [wShowdownConnected]
	and a
	jr z, .normalPlayerConfRNG
	ld a, [wSD_Flags]
	bit 2, a  ; bit 2 = playerConfHitSelf
	jr z, .TriedToUseDisabledMoveCheck
	jr .playerConfHitSelf
.normalPlayerConfRNG
	call BattleRandom
...
.ParalysisCheck
	...
	ld a, [wShowdownConnected]
	and a
	jr z, .normalPlayerParaRNG
	ld a, [wSD_Flags]
	bit 0, a  ; bit 0 = playerCantMove
	jr nz, .PlayerFullyParalyzed
	jr .BideCheck
.normalPlayerParaRNG
	call BattleRandom
	cp 25 percent ; chance to be fully paralyzed
	jr nc, .BideCheck
.PlayerFullyParalyzed
...
CriticalHitTest:
	...
	and a
	jr z, .originalCrit
	callfar ShowdownCriticalHitTest
	ret
.originalCrit
	xor a
	ld [wCriticalHitOrOHKO], a
...
.storeDamage ; store damage value at b
	...
	ld a, b
	ld [hl], a
	; In Showdown mode, override with server's value (fixes Psywave desync)
	ld a, [wShowdownConnected]
	and a
	jr z, ApplyDamageToEnemyPokemon
	callfar ShowdownSetDamage
...
ApplyDamageToEnemyPokemon:
	...
	or b
	jr nz, .checkEnemySub
	; damage is 0 — in Showdown mode, still route to AttackSubstitute if sub is up
	ld a, [wShowdownConnected]
	and a
	jr z, ApplyAttackToEnemyPokemonDone
	ld a, [wEnemyBattleStatus2]
	bit HAS_SUBSTITUTE_UP, a
...
; In Showdown mode, use SubBreakHit to determine if/when sub breaks.
; SubBreakHit = 1-based hit number on which sub breaks (0 = no break this turn).
; For multi-hit moves, the sub absorbs hits before SubBreakHit and breaks on that hit.
	ld a, [wShowdownConnected]
	and a
	jr z, .vanillaDamage
	; Determine M1 or M2 based on hWhoseTurn vs wSD_WhoFirst
	ldh a, [hWhoseTurn]
	ld l, a
	ld a, [wSD_WhoFirst]
	cp l
	jr nz, .sdSubM2
	ld a, [wSD_M1_SubBreakHit]
	jr .sdSubGotBreakHit
.sdSubM2
	ld a, [wSD_M2_SubBreakHit]
.sdSubGotBreakHit
	and a
	ret z            ; SubBreakHit == 0 → sub survives this turn
	ld h, a          ; h = SubBreakHit
	; Determine current hit number
	ld a, l          ; a = hWhoseTurn
	and a
	jr nz, .sdSubEnemy
	; Player attacking
	ld a, [wPlayerBattleStatus1]
	bit ATTACKING_MULTIPLE_TIMES, a
	jr z, .substituteBroke   ; single-hit: SubBreakHit > 0 → break
	ld a, [wPlayerNumHits]
	ld l, a
	ld a, [wPlayerNumAttacksLeft]
	jr .sdSubCalcHit
.sdSubEnemy
	ld a, [wEnemyBattleStatus1]
	bit ATTACKING_MULTIPLE_TIMES, a
	jr z, .substituteBroke
	ld a, [wEnemyNumHits]
	ld l, a
	ld a, [wEnemyNumAttacksLeft]
.sdSubCalcHit
	; l = total hits, a = attacks left
	; current_hit = total - left + 1
	ld e, a
	ld a, l
	sub e
	inc a            ; a = current hit (1-based)
	cp h             ; compare to SubBreakHit
	jr c, .sdSubAbsorb
	jr .substituteBroke
.sdSubAbsorb
	ret
...
MoveHitTest:
	ld a, [wShowdownConnected]
	and a
	jr z, .originalHitTest
	callfar ShowdownMoveHitTest
	ret
.originalHitTest
; player's turn
	ld hl, wEnemyBattleStatus1
...
.notMultiHitMove
	...
	ld a, [wShowdownConnected]
	and a
	jr z, .enemyNormalSideEffects
	; Enemy is executing: if hWhoseTurn == wSD_WhoFirst, this is M1; else M2
	ld a, [hWhoseTurn]
	ld b, a
	ld a, [wSD_WhoFirst]
	cp b
	ld a, [wSD_Flags]
	jr nz, .enemyCheckM2SE
	bit 4, a ; M1 side effect flag
	jr z, ExecuteEnemyMoveDone
	jr .enemyNormalSideEffects
.enemyCheckM2SE
	bit 5, a ; M2 side effect flag
	jr z, ExecuteEnemyMoveDone
.enemyNormalSideEffects
	ld a, [wEnemyMoveEffect]
	and a
	jr z, ExecuteEnemyMoveDone
	ld hl, SpecialEffects
...
.isConfused
	...
	ld a, [wShowdownConnected]
	and a
	jr z, .normalEnemyConfRNG
	ld a, [wSD_Flags]
	bit 3, a  ; bit 3 = enemyConfHitSelf
...
.checkIfParalysed
	...
	ld a, [wShowdownConnected]
	and a
	jr z, .normalEnemyParaRNG
	ld a, [wSD_Flags]
	bit 1, a  ; bit 1 = enemyCantMove
	jr nz, .enemyFullyParalysed
	jr .checkIfUsingBide
.normalEnemyParaRNG
	call BattleRandom
...
LoadEnemyMonData:
	...
	and a                       ; a still holds wLinkState from the cp above
	jp nz, LoadEnemyMonFromParty ; any non-zero link state -> use party data
	ld a, [wEnemyMonSpecies2]
